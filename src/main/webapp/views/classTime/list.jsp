<%@page language="java" contentType="text/html; charset=ISO-8859-1"
	pageEncoding="ISO-8859-1"%>

<%@taglib prefix="jstl" uri="http://java.sun.com/jsp/jstl/core"%>
<%@taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt"%>
<%@taglib prefix="tiles" uri="http://tiles.apache.org/tags-tiles"%>
<%@taglib prefix="spring" uri="http://www.springframework.org/tags"%>
<%@taglib prefix="form" uri="http://www.springframework.org/tags/form"%>
<%@taglib prefix="security"
	uri="http://www.springframework.org/security/tags"%>
<%@taglib prefix="display" uri="http://displaytag.sf.net"%>
<%@taglib prefix="acme" tagdir="/WEB-INF/tags"%>
<link rel="stylesheet" href="styles/horario2.css" type="text/css">

<link href="bootstrap.css" rel="stylesheet" type="text/css" />
<link href="bootstrap-responsive.css" rel="stylesheet" type="text/css" />

<div class="container-fluid">
		<div style="float: right;">
			<a
				href="mailto:?subject=Acme-School&body=El%20codigo%20es%20${parentsGroup.code}%20https://www.acme.com:8443/inscription/parent/createInscription.do?parentsGroupId=${parentsGroup.id}"><img
				src="data:image/png;base64,"
				width="40px" height="40px" /> <spring:message code="share.parent" /><br/> 
			</a>
			<a
				href="mailto:?subject=Acme-School&body=El%20codigo%20es%20${parentsGroup.code}%20https://www.acme.com:8443/inscription/teacher/createInscription.do?parentsGroupId=${parentsGroup.id}"><img
				src="data:image/png;base64,"
				width="40px" height="40px" /> <spring:message code="share.teacher" /><br/> 
	
			</a>
			<a
				href="parentsGroup/parent/invite/teacher.do?parentsGroupId=${parentsGroup.id}&english=<spring:message code="parentsGroup.language.english" />"><img
				src="images/move.png" style="margin:3px; margin-right:0px;"
				width="37px" height="33px" /> <spring:message code="share.teacher.registered" /><br/> 
			</a>
		</div>
		<div>
			<br/>
			<p><b><spring:message code="parentsGroup.name" /></b>:
			<jstl:out value="${parentsGroup.name}" /></p>
			<p><b><spring:message code="classGroup.level" /></b>:
			<jstl:out value="${classGroup.level.level}" /></p>
			<p><b><spring:message code="classGroup.school" /></b>:
			<jstl:out value="${classGroup.level.school.nameSchool}" /></p>
		</div>
		<br/>
		<br/>

	<ul class="nav nav-tabs">
		<li><a href="parentsGroup/parent/display.do?parentsGroupId=${parentsGroupId}"><spring:message code="parentsGroup.chat" /></a></li>
		<li><a href="event/parent/list.do?parentsGroupId=${parentsGroupId}"><spring:message code="parentsGroup.events" /></a></li>
		<li><a href="parent/parent/list.do?parentsGroupId=${parentsGroupId}"><spring:message code="parentsOfGroup.list" /></a></li>
		<li><a href="extracurricularActivity/parent/list.do?parentsGroupId=${parentsGroupId}"><spring:message code="parentsGroup.extracurricular" /></a></li>
		<li><a href="homework/parent/list.do?parentsGroupId=${parentsGroupId}"><spring:message code="parentsGroup.homeworks" /></a></li>
		<li><a href="schoolCalendar/parent/display.do?parentsGroupId=${parentsGroupId}"><spring:message code="parentsGroup.calendar" /></a></li>
		<li class="active"><a href="classTime/parent/list.do?classGroupId=${classGroupId}&&parentsGroupId=${parentsGroupId}"><spring:message code="parentsGroup.schedule" /></a></li>
		
	</ul>
	<br />
	<div class="container-fluid" class="centrado">
		<div class="row-fluid" ></div>

	</div>
	<div class="row-fluid">

		<div class="span2 tiny">
			<div class="pricing-table-header-tiny" style="text-align: center;">
				<h2>
					<spring:message code="classTime.letterday.1" />
				</h2>
				<h3>
					<spring:message code="classTime.day.1" />
				</h3>
			</div>
			<div class="pricing-table-features">
				<jstl:forEach var="x1" items="${mon}">
					<div class="button">
						<a href="classTime/parent/edit.do?classTimeId=${x1.id}&&parentsGroupId=${parentsGroupId}"
							style='text-decoration: none; color: #888;'>
							<p>
								<strong><jstl:out value="${x1.hour }" /></strong><br>
								<jstl:out value="${x1.subject.name }" />
							</p>
						</a>
					</div>
				</jstl:forEach>
				<div class="pricing-table-signup-pro">
					<a
						href="classTime/parent/create.do?classGroupId=${classGroupId}&&day=1&&parentsGroupId=${parentsGroupId}">
						<spring:message code="classTime.create" />
					</a>
				</div>
			</div>
		</div>
		<div class="span2 small">
			<div class="pricing-table-header-small" style="text-align: center;">
				<h2>
					<spring:message code="classTime.letterday.2" />
				</h2>
				<h3>
					<spring:message code="classTime.day.2" />
				</h3>
			</div>
			<div class="pricing-table-features">
				<jstl:forEach var="x2" items="${tue}">
					<div class="button">
						<a href="classTime/parent/edit.do?classTimeId=${x2.id}&&parentsGroupId=${parentsGroupId}"
							style='text-decoration: none; color: #888;'>
							<p>
								<strong><jstl:out value="${x2.hour }" /></strong><br>
								<jstl:out value="${x2.subject.name }" />
							</p>
						</a>
					</div>
				</jstl:forEach>
				<div class="pricing-table-signup-pro">
					<a
						href="classTime/parent/create.do?classGroupId=${classGroupId}&&day=2&&parentsGroupId=${parentsGroupId}">
						<spring:message code="classTime.create" />
					</a>
				</div>
			</div>
		</div>


		<div class="span2 medium">
			<div class="pricing-table-header-medium" style="text-align: center;">
				<h2>
					<spring:message code="classTime.letterday.3" />
				</h2>
				<h3>
					<spring:message code="classTime.day.3" />
				</h3>
			</div>
			<div class="pricing-table-features">
				<jstl:forEach var="x3" items="${wed}">
					<div class="button">
						<a href="classTime/parent/edit.do?classTimeId=${x3.id}&&parentsGroupId=${parentsGroupId}"
							style='text-decoration: none; color: #888;'>
							<p>
								<strong><jstl:out value="${x3.hour }" /></strong><br>
								<jstl:out value="${x3.subject.name }" />
							</p>
						</a>
					</div>
				</jstl:forEach>
				<div class="pricing-table-signup-pro">
					<a
						href="classTime/parent/create.do?classGroupId=${classGroupId}&&day=3&&parentsGroupId=${parentsGroupId}">
						<spring:message code="classTime.create" />
					</a>
				</div>
			</div>
		</div>



		<div class="span2 pro">
			<div class="pricing-table-header-pro" style="text-align: center;">
				<h2>
					<spring:message code="classTime.letterday.4" />
				</h2>
				<h3>
					<spring:message code="classTime.day.4" />
				</h3>
			</div>
			<div class="pricing-table-features">
				<jstl:forEach var="x4" items="${thu}">
					<div class="button">
						<a href="classTime/parent/edit.do?classTimeId=${x4.id}&&parentsGroupId=${parentsGroupId}"
							style='text-decoration: none; color: #888;'>
							<p>
								<strong><jstl:out value="${x4.hour }" /></strong><br>
								<jstl:out value="${x4.subject.name }" />
							</p>
						</a>
					</div>
				</jstl:forEach>
				<div class="pricing-table-signup-pro">
					<a
						href="classTime/parent/create.do?classGroupId=${classGroupId}&&day=4&&parentsGroupId=${parentsGroupId}">
						<spring:message code="classTime.create" />
					</a>
				</div>
			</div>
		</div>
		<div class="span2 tiny">
			<div class="pricing-table-header-tiny" style="text-align: center;">
				<h2>
					<spring:message code="classTime.letterday.5" />
				</h2>
				<h3>
					<spring:message code="classTime.day.5" />
				</h3>
			</div>
			<div class="pricing-table-features">
				<jstl:forEach var="x5" items="${fri}">
					<div class="button">
						<a href="classTime/parent/edit.do?classTimeId=${x5.id}&&parentsGroupId=${parentsGroupId}"
							style='text-decoration: none; color: #888;'>
							<p>
								<strong><jstl:out value="${x5.hour }" /></strong><br>
								<jstl:out value="${x5.subject.name }" />
							</p>
						</a>
					</div>
				</jstl:forEach>
				<div class="pricing-table-signup-pro">
					<a
						href="classTime/parent/create.do?classGroupId=${classGroupId}&&day=5&&parentsGroupId=${parentsGroupId}">
						<spring:message code="classTime.create" />
					</a>
				</div>
			</div>
		</div>
	</div>
	<div class="row-fluid"></div>
</div>


<!-- Botón de create -->

</div>

