...
<div class="container-fluid">
	...
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="event/parent/list.do?parentsGroupId=${parentsGroup.id}"><spring:message code="parentsGroup.events" /></a></li>
...
<display:table pagesize="5" class="table table-striped table-bordered" name="event"
	requestURI="event/parent/list.do" id="rowEvent">

	<spring:message code="event.Date.format" var="eventDateFormat" />
	
	<display:column property="moment"  titleKey="event.moment" format ="{0,date,${eventDateFormat}}"/>
		
	<display:column titleKey="event.location">
		<jstl:if test="${not empty rowEvent.location.eastCoordinate and not empty rowEvent.location.northCoordinate}">
		<jstl:out value="${rowEvent.location.northCoordinate}" />, <jstl:out value="${rowEvent.location.eastCoordinate}" />
		</jstl:if>
	</display:column>
	
	<display:column titleKey="event.description">
		<jstl:out value="${rowEvent.description}" />
	</display:column>
	
	<display:column titleKey="event.edit">
		<a href="event/parent/edit.do?eventId=${rowEvent.id }"><spring:message
			code="event.edit" /></a>
	</display:column>
	
	<display:column titleKey="event.display">
		<a href="event/parent/display.do?eventId=${rowEvent.id }"><spring:message
			code="event.display" /></a>
	</display:column>

</display:table>

<div>
	<a href="event/parent/create.do?parentsGroupId=${parentsGroupId}"><spring:message
			code="event.create" /></a>
</div>
